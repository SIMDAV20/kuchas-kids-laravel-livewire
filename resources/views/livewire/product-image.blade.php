    ...
    <div class="py-4 px-2 text-center">
        ...
        <h1 class="text-lg font-semibold mb-2 text-gray-550">
            <a :class="p_color == {{ $key }} ? '' : 'hidden'"
                href="{{ route('products.show', ['product' => $product, 'color' => $p_color_prod->color->slug]) }}">
                {{ Str::limit($product->name, 40) }}
            </a>
        </h1>
        @empty
        <h1 class="text-lg font-semibold mb-2 text-gray-550">
            <a href="{{ route('products.show', ['product' => $product]) }}">{{ Str::limit($product->name, 25) }}</a>
        </h1>
        @endforelse
        ...
        @endif
        @endif
        {{-- @if ($low_price < 9999999) @else @if ($product->offer_price > 0 &&
            \Carbon\Carbon::parse($product->offer_date)->format('Y-m-d') >= \Carbon\Carbon::now()->format('Y-m-d') &&
