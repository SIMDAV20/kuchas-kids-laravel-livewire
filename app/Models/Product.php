...
class Product extends Model
{
    ...
    {
        $basePrice = $this->offer_price > 0 ? $this->offer_price : ($this->price ?? 0);

        if ($this->variants()->count() > 0) {
            $effectiveMin = $this->variants()->get()->map(function ($variant) use ($basePrice) {
                if ($variant->offer_price > 0) return $variant->offer_price;
                return $variant->price > 0 ? $variant->price : $basePrice;
            })->min();

            return $effectiveMin ?? $basePrice;
        }
